...
for i in range(len(StreamingPorts)):
  ...
  udp_thread = threading.Thread(target=udp_functions.kafka_slim_single_thread_udp_receiver_MultiMerlin, args=((lambda: stop_threads), PACKET_COUNT[i], i, int(StreamingPorts[i]), StreamingIPs[i]))
  udp_thread.start()
